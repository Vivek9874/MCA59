...
-- stored procedures --
CALL Get_Order_Status('Shipped');
CALL Get_Order_Status('Cancelled');
CALL Get_Order_Status('On Hold');

-- functions --
-- 2.  Create a function which will return city of the given officeCode --
 select GetCity(3) as cityname;
 
 -- 3. Create a function to show the highest MSRP for each productline using window functions --
 select productLine, max(MSRP) as highestmsrp from products group by productLine;	
select HighMSRP('Planes') as MaxMSRP;
select HighMSRP ('Classic Cars')as MaxMSRP;
select HighMSRP('motorcycles')as MaxMSRP;
select HighMSRP('Planes')as MaxMSRP;
select HighMSRP('Ships')as MaxMSRP;
select HighMSRP('Trains')as MaxMSRP;
select HighMSRP('Trucks and Buses')as MaxMSRP;
select HighMSRP('Vintage Cars')as MaxMSRP;
select GetHighMSRP('Planes') as MaxMSRP;
USE classicmodels;
-- 4. Create a function to show the customername who has used the highest CreditLimit. --
select HighCRED();
SELECT MAX(MSRP) over (PARTITION BY productLine) 
    FROM products
